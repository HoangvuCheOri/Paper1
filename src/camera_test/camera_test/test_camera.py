...
class CameraTestNode(Node):
    def __init__(self):
        ...
        self.stream_thread = threading.Thread(target=self.camera_stream_thread, daemon=True)
        self.stream_thread.start()

        self.timer = self.create_timer(1.0 / 30.0, self.process_frame)
        self.get_logger().info("Camera Test Node Started. Publishing RAW pose to /apriltag_pose")
    ...
    def camera_stream_thread(self):
        ...
        while rclpy.ok():
            if cap is None or not cap.isOpened():
                ...
                if not cap.isOpened():
                    self.get_logger().warn("Cannot connect to camera, retrying in 1s...")
                    time.sleep(1)
                    continue
    ...
    def process_frame(self):
        ...
        # Xử lý trường hợp luồng stream bị giảm độ phân giải (VD: 640x480)
        scale_x = w_in / 1280.0
        scale_y = h_in / 720.0
        ...
        # Undistort frame (chỉ khi scale=1, hoặc có thể dùng frame nguyên gốc)
        ...
        for det in detections:
            ...
            # --- TÍNH TỌA ĐỘ BÙ NGHIÊNG TRÊN MẶT SÀN CỐ ĐỊNH ---
            ...
            R_floor2cam = R_orig @ Rz_yaw.T  # Ma trận này cố định, không đổi khi robot quay tại chỗ
            
            # Chiếu tọa độ tvec xuống mặt phẳng sàn cố định
            pos_floor = R_floor2cam.T @ tvec
            
            # pos_floor[0] = sang phải màn hình (Camera X)
            # pos_floor[1] = xuống dưới màn hình (Camera Y)
            # Hệ robot: X hướng lên (-pos_floor[1]), Y hướng trái (-pos_floor[0])
            # Thực nghiệm: Y và Yaw chuẩn, nhưng tiến 1.0m thì X báo 1.2m
            # Do đó phải chia X cho 1.2 để bù lại phần hình bị giãn dọc
            robot_x = (-pos_floor[1, 0]) / 1.2
            robot_y = -pos_floor[0, 0]
            
            yaw_deg = normalize_angle_deg(-(math.degrees(yaw_cam) + 90.0))
            
            # KHÔNG dùng Kalman Filter, lấy RAW
            pose_msg = Pose2D()
            ...
            # Hiển thị terminal cho tiện theo dõi
            self.get_logger().info(f"RAW -> X: {robot_x:.3f} m, Y: {robot_y:.3f} m, Yaw: {yaw_deg:.1f} deg")
            ...
            # Ghi text tọa độ ngay cạnh Tag
            tag_center_x = int(np.mean([p[0] for p in img_pts]))
            tag_center_y = int(np.mean([p[1] for p in img_pts]))
            cv2.putText(frame, f"X:{robot_x:.2f} Y:{robot_y:.2f}", (tag_center_x, tag_center_y - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            cv2.putText(frame, f"Yaw:{yaw_deg:.1f}", (tag_center_x, tag_center_y + 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            break

        # --- VẼ GỐC TỌA ĐỘ TÂM CAMERA ---
        cx, cy = w_in // 2, h_in // 2
        # Vẽ dấu cộng (+) màu đỏ tại tâm (điểm 0,0)
        cv2.line(frame, (cx - 20, cy), (cx + 20, cy), (0, 0, 255), 2)
        cv2.line(frame, (cx, cy - 20), (cx, cy + 20), (0, 0, 255), 2)
        cv2.putText(frame, "ORIGIN (0,0)", (cx + 10, cy - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        
        # Vẽ hướng trục X (Lên) và Y (Trái) từ tâm
        cv2.arrowedLine(frame, (cx, cy), (cx, cy - 100), (255, 0, 0), 3, tipLength=0.1) # X xanh dương
        cv2.putText(frame, "+X", (cx - 30, cy - 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        
        cv2.arrowedLine(frame, (cx, cy), (cx - 100, cy), (255, 0, 255), 3, tipLength=0.1) # Y tím
        cv2.putText(frame, "+Y", (cx - 90, cy - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
