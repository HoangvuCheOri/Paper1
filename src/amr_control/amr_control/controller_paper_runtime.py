...
def automatic_duration(node, trajectory):
    """Return startup + settling + one complete trajectory + a small buffer."""
    startup = max(0.0, float(getattr(node, "STARTUP_DELAY", 1.0)))
    settling = max(0.0, float(getattr(node, "SETTLE_TIME", 2.0)))
    if trajectory == "circle":
        motion = 2.0 * math.pi / max(abs(float(node.W)), 1e-6)
        ...
        motion = float(node.loop_length) / max(float(node.VD), 1e-6)
    else:
        raise ValueError(f"Unsupported trajectory: {trajectory}")
    return startup + settling + motion + 2.0
...
def run_controller(node_factory, controller_label, trajectory, configure=None, args=None):
    ...
    try:
        if configure is not None:
            configure(node)
        duration = automatic_duration(node, trajectory)
